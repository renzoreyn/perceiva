...
export async function createTransaction(input: TransactionInput): Promise<{ success: boolean; error?: string }> {
  ...
  const dbUser = await prisma.user.findUnique({ where: { id: user.id }, select: { baseCurrency: true } });
...
export async function getTransactions(options?: {
  limit?: number; offset?: number; type?: "income" | "expense";
  category?: string; currency?: string; search?: string; tripId?: string;
}) {
  const { user } = await validateRequest();
  if (!user) throw new Error("Not authenticated");

  const { limit = 200, offset = 0, type, category, currency, search, tripId } = options ?? {};
...
// Called from page.tsx — userId passed in to avoid re-validating session during render
export async function getDashboardDataForUser(userId: string, baseCurrency: string) {
  ...
  const [recentTransactions, incomeAgg, expenseAgg, prevExpenseAgg, categoryBreakdown, currencyBreakdown] =
    await Promise.all([
      ...
      prisma.transaction.aggregate({
        where: { userId, type: "income", createdAt: { gte: thirtyDaysAgo } },
      ...
      prisma.transaction.aggregate({
        where: { userId, type: "expense", createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } },
        _sum: { convertedAmount: true },
      }),
  ...
  const currentDailyRate = dayOfMonth > 0 ? totalExpense / dayOfMonth : 0;
  const prevDailyRate = prevExpense / 30;
  const velocityPct = prevDailyRate > 0 ? ((currentDailyRate - prevDailyRate) / prevDailyRate) * 100 : 0;
  ...
  return {
    ...
    velocity: { pct: velocityPct, faster: velocityPct > 5, slower: velocityPct < -5 },
    recentTransactions,
    categoryBreakdown: categoryBreakdown.map((c) => ({ category: c.category, total: Number(c._sum.convertedAmount ?? 0) })),
...
// Keep old getDashboardData for backward compat
export async function getDashboardData() {
  const { user } = await validateRequest();
  if (!user) throw new Error("Not authenticated");
  const dbUser = await prisma.user.findUnique({ where: { id: user.id }, select: { baseCurrency: true, name: true, email: true } });
  const data = await getDashboardDataForUser(user.id, dbUser?.baseCurrency ?? "USD");
  return { user: dbUser, ...data };
}
...
export async function exportTransactionsCSV(): Promise<string> {
  ...
  const header = ["Date","Type","Amount","Currency","Converted Amount","Base Currency","Exchange Rate","Category","Description","Recurring","Trip"].join(",");
