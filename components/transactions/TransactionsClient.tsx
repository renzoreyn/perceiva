      ...
      <main className="flex-1 overflow-auto relative z-10 dashboard-main">
        <div style={{ maxWidth: "920px", margin: "0 auto", padding: "40px 24px" }}>
          ...
          <div className="flex items-center justify-between" style={{ marginBottom: "28px" }}>
            <div>
              <h1 className="font-playfair text-3xl font-semibold" style={{ color: "var(--t1)", marginBottom: "4px" }}>
                Transactions
              </h1>
          ...
          <div className="glass glass-shimmer rounded-2xl p-4 mb-5 flex flex-wrap items-center gap-3">
            ...
            <div className="flex rounded-lg p-0.5" style={{ background: "rgba(255,255,255,0.05)" }}>
              {(["all", "expense", "income"] as const).map((f) => (
                <button key={f} onClick={() => setTypeFilter(f)}
                  ...
                  style={{
                    background: typeFilter === f ? "rgba(108,144,216,0.2)" : "transparent",
                    color: typeFilter === f ? "var(--t-accent)" : "var(--t3)",
            ...
            <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}
              ...
              style={{ background: "rgba(255,255,255,0.05)", border: "1px solid var(--border)", color: categoryFilter ? "var(--t1)" : "var(--t3)" }}
            >
            ...
            <select value={currencyFilter} onChange={(e) => setCurrencyFilter(e.target.value)}
              className="px-3 py-2 rounded-lg text-xs outline-none appearance-none"
              style={{ background: "rgba(255,255,255,0.05)", border: "1px solid var(--border)", color: currencyFilter ? "var(--t1)" : "var(--t3)" }}
            >
            ...
            {hasFilters && (
              <button onClick={clearFilters}
                ...
                style={{ background: "rgba(224,107,107,0.1)", color: "var(--red)", border: "1px solid rgba(224,92,92,0.2)", cursor: "pointer" }}
              >
                <X size={11} /> Clear
              </button>
            )}
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center py-16 text-center">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-3" style={{ background: "rgba(255,255,255,0.04)" }}>
                  <Filter size={18} style={{ color: "var(--t3)" }} />
                </div>
                <p className="text-sm font-medium mb-1" style={{ color: "var(--t2)" }}>No transactions found</p>
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-sm font-medium" style={{ color: tx.type === "income" ? "var(--green)" : "var(--red)" }}>
